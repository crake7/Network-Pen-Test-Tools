...
def handler(chan, host, port):
    # Open the socket on the target host and port 
    sock = socket.socket()
    try:
    ...
    while True:
        # A file descriptor is either a socket or a file object
...
def reverse_forward_tunnel(server_port, remote_host, remote_port, transport):
    # Forward TPC connections from a listening port to this SSH session
    transport.request_port_forward("", server_port)
    while True:
        # Start a new transport channel
        channel = transport.accept(1000)
        if channel is None:
            continue
        thr = threading.Thread(
            target=handler, args=(channel, remote_host, remote_port)
        )
        thr.setDaemon(True)
        thr.start()
...
def main():
    # Checks arguments were passed correctly.
    ...
    # Open a session with an SSH server
    client = paramiko.SSHClient()
    # Attempts to read HOSTKEYS from local 'known hosts' file
    client.load_system_host_keys()
    # Warns an unknown host key, but accepting it
